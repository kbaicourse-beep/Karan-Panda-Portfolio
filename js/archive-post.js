...
function renderArchivePost() {
  ...
  if (indexLabel) {
    indexLabel.innerHTML = `<span style="color:var(--accent)">${post.indexLabel}</span> <span style="color:var(--t-60)">— ${post.tag.toUpperCase()}</span>${post.isNew ? ` <span class="new-badge new-badge-inline">New</span>` : ""}`;
  }
